...
interface ProspeccaoAtualizaProps {
  prospeccaoId: string | null;
}

export default function ProspeccaoAtualizar({
  prospeccaoId,
}: ProspeccaoAtualizaProps) {
  ...
  const { data: prospeccao } = useQuery(["prospecccoes"], async () => {
    const response = await api.get(`/prospects/${prospeccaoId}`);    

    return response.data;
  });

  useEffect(() => {
    if (prospeccao) reset(prospeccao);    
  }, [reset, prospeccao]);  
  ...
  const mutation = useMutation(postProspeccao, {
    onSuccess: () => {
      snackbar.success("Prospecção atualizada com sucesso!");
      setTimeout(() => navigate("/prospects"), 1000);
    },
    onError: () => {
      snackbar.error(`Erro ao atualizar prospecção: ${mutation.error}`);
    },
  });
            ...
            <Controller
              ...
              render={({ field:{ value, onChange} }) => (
                <Select
                  ...
                  options={options}
                  mapOptionToLabel={(seguimento: any) =>
                  ...
                  >
                    {listaCidade.map(
                      (
                        ...
                        <option value={`${a.nomeCidade}`} key={`${a.nomeCidade}`}>
                          {a.nomeCidade}
                        </option>
                      )
                    )}
        ...
        <Row>
            ...
            <Controller 
              ...
              render={({ field: { value, onChange} }) => (
                <InputMask
                  ...
                  onChange={onChange}
                >
                  <Input id="proximoContato" name="proximoContato" labelInput="Próx. Contato" type="text" />
                </InputMask>
              )}
            />
          </Col>
          <Col xs={12} sm={12} md={12} lg={6} xl={6}>
            <Controller 
              ...
              control={control}
              render={({ field }) => (
                <Input id="nomeSistema" labelInput="Nome Sistema" type="text" {...field} />
              )}
            />
          </Col>
          <Col xs={12} sm={12} md={12} lg={2} xl={2}>
            <Controller 
              ...
              control={control}
              render={({ field: { value, onChange} }) => (
                <Checkbox idCheckbox="ativo" textCaption="Ativo" checked={value} onChange={onChange} />
              )}
            />
          </Col>
        </Row>
